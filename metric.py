...
def inference(loader, model, device, view, data_size):
    ...
    for step, (xs, y, _) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            hs, _, _, zs,z,label = model.forward(xs)
            q = sum(qs)/view
            z=z.detach()
        for v in range(view):
            hs[v] = hs[v].detach()
    ...
    commonZ=torch.cat(commonZ, dim=0)


    labels = np.argmax(np.array(labels), axis=1)

    commonZ=np.array(commonZ.cpu().detach().numpy())
    ...
    return total_pred, pred_vectors, Hs, labels_vector, Zs,commonZ,labels


def valid(model,args,device, dataset, view, data_size):
    global kmeans
    ...
    total_pred, pred_vectors, high_level_vectors, labels_vector, low_level_vectors,commonZ,labels = inference(test_loader, model, device, view, data_size)


    print("Clustering results on semantic labels: " + str(labels_vector.shape[0]))
    nmi, ari, acc, pur = evaluate(labels_vector, labels)
